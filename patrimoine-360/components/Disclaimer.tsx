...
export default function Disclaimer() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-navy-900/95 border-t border-gold-500/20 backdrop-blur-sm px-4 py-3">
      <div className="max-w-5xl mx-auto flex items-start gap-3">
        <Info size={18} className="text-gold-400 flex-shrink-0 mt-0.5" />
        <p className="text-xs text-navy-200 flex-1">
        ...
        <button onClick={() => setVisible(false)} className="text-gold-400 hover:text-white transition flex-shrink-0">
          <X size={16} />
        </button>
      </div>
    </div>
