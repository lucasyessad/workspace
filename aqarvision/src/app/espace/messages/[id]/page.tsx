    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // Mark as read if message from agent
          supabase
            .from("conversations")
            .update({ visiteur_non_lu: 0 })
            .eq("id", conversationId);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 bg-white rounded-t-2xl">
        <button
          onClick={() => router.push("/espace/messages")}
          className="p-1 rounded-lg hover:bg-gray-100 transition-colors"
        >
          <ArrowLeft className="h-5 w-5 text-[#0c1b2a]" />
        </button>
        <div className="w-8 h-8 rounded-full bg-[#0c1b2a] flex items-center justify-center flex-shrink-0 overflow-hidden">
          {conversation?.agent?.logo_url ? (
            <img
              src={conversation.agent.logo_url}
              alt={conversation.agent.nom_agence}
              className="w-full h-full object-cover"
            />
          ) : (
            <span className="text-xs font-bold text-white">
              {conversation?.agent?.nom_agence?.charAt(0)?.toUpperCase() ?? "A"}
            </span>
          )}
        </div>
        <div className="min-w-0">
          <p className="text-sm font-semibold text-[#0c1b2a] truncate">
            {conversation?.agent?.nom_agence ?? "Agence"}
          </p>
