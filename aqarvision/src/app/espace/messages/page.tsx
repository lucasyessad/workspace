  ...
  useEffect(() => {
    async function fetchConversations() {
      ...
      const { data } = await supabase
        .from("conversations")
        .select(
          "*, agent:profiles(nom_agence, logo_url), listing:listings(titre, photos)"
        )
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-bold text-[#0c1b2a]">Mes messages</h1>
      </div>
      ...
      {conversations.length === 0 ? (
        ...
        <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden divide-y divide-gray-100">
          {conversations.map((conv) => (
            <Link
              key={conv.id}
              href={`/espace/messages/${conv.id}`}
              className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition-colors"
            >
              {/* Agent avatar */}
              <div className="w-10 h-10 rounded-full bg-[#0c1b2a] flex items-center justify-center flex-shrink-0 overflow-hidden">
                {conv.agent?.logo_url ? (
                  <img
                    src={conv.agent.logo_url}
                    alt={conv.agent.nom_agence}
                    className="w-full h-full object-cover"
                  />
                ) : (
                  <span className="text-sm font-bold text-white">
                    {conv.agent?.nom_agence?.charAt(0)?.toUpperCase() ?? "A"}
                  </span>
                )}
              </div>
